...
import { UserPlus, ChevronDown, ChevronUp, Upload, ArrowLeft } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea, Select } from "@/components/ui/textarea";
import { createIntake, saveIntake, getClientIntake, IntakeData } from "@/lib/intake-store";
...
export default function ClientIntakePage() {
  ...
  const [existingIntake, setExistingIntake] = useState<IntakeData | undefined>(undefined);
  ...
  const [form, setForm] = useState<IntakeData>(createIntake());
          ...
          setForm((prev) => ({
            ...
            personal: {
              ...
              dateOfBirth: data.client.dateOfBirth
                ? new Date(data.client.dateOfBirth).toISOString().split("T")[0]
                : "",
            },
            address: {
              ...prev.address,
              street: data.client.addressStreet || "",
              city: data.client.addressCity || "",
              state: data.client.addressState || "",
              zip: data.client.addressZip || "",
            },
          }));
        }
        setLoading(false);
  ...
  useEffect(() => {
    if (!clientId) return;
    getClientIntake(clientId).then((data) => {
      if (data) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/intake", {
        ...
        body: JSON.stringify({ formData: form, clientId }),
      });
      if (!res.ok) {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-16">
        ...
        <p className="text-slate-500 mt-2">
          Intake data for {client.firstName} {client.lastName} has been saved. The data is now available for I-485 form preparation.
        </p>
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        <Link href={`/clients/${clientId}`}>
          <Button variant="ghost" size="sm"><ArrowLeft size={16} /> Back to Client</Button>
        </Link>
      </div>

      <div>
        <h1 className="text-2xl font-bold text-slate-900">
          {existingIntake ? "Edit Intake" : "New Intake"} — {client.firstName} {client.lastName}
        </h1>
        <p className="text-sm text-slate-500 mt-1">
          {existingIntake
            ? "Update existing intake data. Changes will create a new intake version."
        ...
        <CardContent className="p-6">
          ...
          <div className="flex justify-end gap-3 mt-6">
            <Button variant="primary" size="lg" onClick={handleSubmit} disabled={saving}>
              <UserPlus size={16} /> {saving ? "Saving..." : existingIntake ? "Update Intake" : "Submit Intake"}
            </Button>
          </div>
